...
```text
锁定身份、体积、朝向与落点 → 保留三个线索 → 重构一个精致等距微缩实体 → 从源图内容提炼一个主题空间切片 → 让厚涂成为水面、地面、道路、草地、光带、岸线或影子 → 建立真实落地、嵌入、穿越、倒影、波纹、投影或越界 → 保留暖白纸呼吸 → 让文案对齐透视与颜料边缘
```
...
完整审美约束与拒绝项写在 Skill 和生产提示词中；它们保留原始提示词的审美动机，但不会把历史 3:4 画布变成隐藏默认值。 [SKILL.md](SKILL.md) · [production prompt](references/xxd-panel-047-prompt.en.md)
...
| 模式 | 画幅逻辑 | 成品 |
| --- | --- | --- |
| `top-bottom` | 用户确认的整张成品画幅 | 一次生成完整画布：高保真原图在上，047 设计在下，约 50/50 |
| `left-right` | 用户确认的整张成品画幅 | 一次生成完整画布：高保真原图在左，047 设计在右，约 50/50 |
| `design-only` | 用户确认的整张成品画幅 | 047 设计铺满画布，不显示原照片 |
...
本项目的文案规则： 使用一个简短主体、地点、情绪或象征标题，只配必要副文、用户提供编号或微注释；目标语言文字沿等距方向、厚涂边缘、透视路径或暖白留白排列，必须呼应空间而不是悬浮成独立标题。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/047-source.md) · [英文生产提示词](references/xxd-panel-047-prompt.en.md) · [中文生产提示词](references/xxd-panel-047-prompt.zh-CN.md)
...
<div align="center"><strong>主体不是放在颜料上，而是进入颜料生成的世界。</strong></div>
